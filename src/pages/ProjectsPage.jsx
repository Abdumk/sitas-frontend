...
function ProjectsPage() {
  ...
  const handleSubmit = (e) => {
    ...
    if (editId) {
      axios
        .put(`${API_URL}/projects/${editId}`, form, { headers: { Authorization: `Bearer ${token}` } })
        .then(() => {
          fetchProjects();
          resetForm();
        });
      ...
      axios
        .post(`${API_URL}/projects`, form, { headers: { Authorization: `Bearer ${token}` } })
        .then(() => {
          fetchProjects();
          resetForm();
        });
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this project?')) {
      axios
        .delete(`${API_URL}/projects/${id}`, { headers: { Authorization: `Bearer ${token}` } })
        .then(() => fetchProjects());
    }
  };
      ...
      <div className={styles.cards}>
        {filteredProjects.map((project) => (
          <div key={project._id} className={styles.card}>
            ...
            <div className={styles.cardInfo}>
              <strong>Status:</strong>
              <span className={project.status === 'ongoing' ? styles.ongoing : styles.completed}>{project.status}</span>
            </div>
            <div className={styles.cardActions}>
              <button onClick={() => handleEdit(project)}>Edit</button>
              <button onClick={() => handleDelete(project._id)} className={styles.deleteBtn}>
        ...
        {filteredProjects.length === 0 && <p className={styles.empty}>No projects found.</p>}
      </div>
    </div>
  );
}
